...
expanded to AND/inverter by aigmap) and of n chained mux-based full adders.
  ...
  python3 -B BDD_MUX_FULL_ADDER.py MUX_FULL_ADDER.aag   -> verify your 1-bit mux full adder
  python3 -B BDD_MUX_FULL_ADDER.py                      -> scaling: n = 1..128 chained internally
  python3 -B BDD_MUX_FULL_ADDER.py 8 16 32              -> scaling for the widths you choose

Method: build ROBDDs (ITE) for every gate, then for the outputs; build a reference adder in the
same BDD manager; BDDs are canonical, so the circuit is correct iff the output BDDs are the SAME
nodes. Variable order: cin, a0, b0, a1, b1, ...

Note: aigmap decomposes each 2:1 MUX into AND/inverter gates (same as other adders), so this file
reuses the general AND-gate AIG builder for the scaling chain (functionally identical to the
mux_full_adder module, just expressed with AND/OR/XOR primitives for the chain generator).
"""#!/usr/bin/env python3
"""
BDD-based formal verification of the RIPPLE CARRY ADDER (your fa module, chained by Yosys).

  python3 -B BDD_RCA.py RCA.aag          -> verify an AIG you already generated
  python3 -B BDD_RCA.py                  -> scaling: Yosys builds n = 2..128 bit RCAs (no ABC)
  python3 -B BDD_RCA.py 8 16 32          -> scaling for the widths you choose

Scaling needs in the same folder:  FA.v  RCA_PARAM.v      (top module: rca)
...
# Chain of n full adders (functionally identical to mux_full_adder, chained), 9 AND gates each.
def build_chain(n):
    g = AIG(); g.ands = []; nxt = [1]
    def new():

#!/usr/bin/env python3
"""
BDD-based formal verification of the MUX-NATIVE FULL ADDER (Yosys AIG, MUX cells
expanded to AND/inverter by aigmap) and of n chained mux-based full adders.

  python3 -B MUX_FULL_ADDER_BDD.py MUX_FULL_ADDER.aag   -> verify your 1-bit mux full adder
  python3 -B MUX_FULL_ADDER_BDD.py                      -> scaling: n = 1..512 chained internally
  python3 -B MUX_FULL_ADDER_BDD.py 8 16 32               -> scaling for the widths you choose

Method: build ROBDDs (ITE) for every gate, then for the outputs; build a reference
adder in the same BDD manager; BDDs are canonical, so the circuit is correct iff the
output BDDs are the SAME nodes. Variable order: cin, a0, b0, a1, b1, ...

IMPORTANT (fixed vs. the earlier version of this script): the scaling chain below is
now a TRUE per-bit expansion of the 5 real mux2 instances used in MUX_FULL_ADDER.v
(M_XOR1, M_XOR2, M_AND, M_OR, M_COUT) -> 5 muxes x 3 AND gates = 15 AND gates per
bit, exactly matching MUX_FULL_ADDER.aag/.aig. The earlier version silently used a
generic, already-reduced AND/XOR/OR builder (9 AND gates/bit) for the scaling table,
so it never actually exercised the 5-mux structure at width > 1 -- only the
single-file .aag/.aig path did. That is corrected here.
"""
...
# TRUE chain of n MUX_FULL_ADDER bits: each bit is the actual 5-mux structure from
# MUX_FULL_ADDER.v (M_XOR1, M_XOR2, M_AND, M_OR, M_COUT), each mux expanded to its
# own 3 AND gates -> 15 AND gates per bit, matching MUX_FULL_ADDER.aag/.aig exactly.
def build_chain(n):
    g = AIG(); g.ands = []; nxt = [1]
    def new():
        v = nxt[0]; nxt[0] += 1; return 2 * v
    def AND(x, y):
        o = new(); g.ands.append((o, x, y)); return o
    def NOT(x): return x ^ 1
    def OR(x, y): return NOT(AND(NOT(x), NOT(y)))
    def MUX(sel, d1, d0): return OR(AND(sel, d1), AND(NOT(sel), d0))          # y = sel ? d1 : d0
    ...
    outs, c = [], cin
    for i in range(n):
        p    = MUX(A[i], NOT(B[i]), B[i])   # M_XOR1: p    = a XOR b
        s    = MUX(c, NOT(p), p)            # M_XOR2: sum  = cin XOR p
        ab   = MUX(A[i], B[i], 0)           # M_AND:  ab   = a AND b     (0 = const false)
        aorb = MUX(A[i], 1, B[i])           # M_OR:   aorb = a OR b     (1 = const true)
        cy   = MUX(c, aorb, ab)             # M_COUT: cout = MAJ(a,b,cin)
        outs.append(s); c = cy
    outs.append(c)

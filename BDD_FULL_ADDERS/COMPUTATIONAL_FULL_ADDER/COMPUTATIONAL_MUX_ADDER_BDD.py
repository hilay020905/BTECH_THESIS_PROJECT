#!/usr/bin/env python3
"""
BDD-based formal verification of the COMPUTATIONAL_MUX_ADDER (Yosys-style AIG, MUX
cells expanded to AND/inverter) and of n chained instances of it.

  python3 -B COMPUTATIONAL_MUX_ADDER_BDD.py COMPUTATIONAL_MUX_ADDER.aag   -> verify the 1-bit adder
  python3 -B COMPUTATIONAL_MUX_ADDER_BDD.py                                -> scaling: n = 1..512 chained
  python3 -B COMPUTATIONAL_MUX_ADDER_BDD.py 8 16 32                        -> scaling for chosen widths

This design shares a single XOR term p = A xor B between both outputs, instead of
building sum/cout from five independent muxes:

    p    = A xor B
    Cout = ITE(p, Ci, A)      i.e.  p ? cin : a
    S    = ITE(p, ~Ci, Ci)    i.e.  p ? ~cin : cin

That is only 3 muxes per bit (1 shared + 2 output muxes) instead of 5, which lowers
the AND-gate count after mux -> AND/inverter expansion (9 AND gates per bit here vs
15 for the original MUX_FULL_ADDER).

Method: build ROBDDs (ITE) for every gate in the parsed AIG, then build a reference
adder directly from the ITE formulas above in the SAME BDD manager; BDDs are
canonical, so the circuit is correct iff the output BDD nodes are identical to the
reference nodes. Variable order: cin, a0, b0, a1, b1, ...
"""
...
def _bdd_verify(g):
    ...
    # reference adder built with the SAME ITE formulas as the circuit's own design
    # (image):   p    = A xor B
    #            Cout = ITE(p, Ci, A)
    #            S    = ITE(p, ~Ci, Ci)
    # built in the same manager; BDDs are canonical -> compare node ids
    Aid = [m.mk(lev[x], 0, 1) for x in d["A"]]; Bid = [m.mk(lev[x], 0, 1) for x in d["B"]]
    c, ok = m.mk(lev[d["cin"]], 0, 1), True
    for i in range(n):
        p  = m.ite(Aid[i], m.ite(Bid[i], 0, 1), Bid[i])                        # p = A xor B
        s  = m.ite(p, m.ite(c, 0, 1), c)                                        # S    = ITE(p, ~Ci, Ci)
        cy = m.ite(p, c, Aid[i])                                                # Cout = ITE(p, Ci, A)
...
# Chain of n COMPUTATIONAL_MUX_ADDER bits: each bit shares one XOR (p) between its
# sum and cout muxes, giving 9 AND gates per bit (vs 15 for the 5-mux design).
def build_chain(n):
    g = AIG(); g.ands = []; nxt = [1]
    def new():
    ...
    outs, c = [], cin
    for i in range(n):
        p    = MUX(A[i], NOT(B[i]), B[i])   # p    = A xor B          (M_P)
        s    = MUX(p, NOT(c), c)            # S    = ITE(p, ~Ci, Ci)  (M_SUM)
        cy   = MUX(p, c, A[i])              # Cout = ITE(p, Ci, A)    (M_COUT)
        outs.append(s); c = cy
    outs.append(c)
